...
with open('anomaly.csv') as csvfile:
    readCSV = csv.reader(csvfile, delimiter=',')
    datavals=[]
    for row in readCSV:
        datavals.append([float(row[3]),float(row[2]),float(row[0]),float(row[4])])

abr=ABNormality(g,datavals)
print(abr)   
abr=ABNormality(g,clustermat[0])
print(abr)
"""
plt.plot([item[0] for item in datavals], [item[1] for item in datavals], 'o', markerfacecolor=tuple(col), 
              markeredgecolor='k', markersize=14)
plt.plot([item[0] for item in clustermat[0]], [item[1] for item in clustermat[0]], 'o', markerfacecolor='blue', 
              markeredgecolor='k', markersize=14)
